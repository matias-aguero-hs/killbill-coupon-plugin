...
public class GetAllAccountsWithCouponServlet extends PluginServlet {

    private final LogService logService;
    private final CouponPluginApi couponPluginApi;

    public GetAllAccountsWithCouponServlet(final LogService logService, final CouponPluginApi couponPluginApi)
    {
    ...
    /**
     * Method doGet will handle the get All Accounts with Coupon operation
     * @param request
     * @param response
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (null != couponCode && !couponCode.isEmpty()) {
            try {
                logService.log(LogService.LOG_INFO, "Calling CouponPluginAPI to get all Coupons Applied by Coupon code.");
                List<CouponsAppliedRecord> couponsApplied = couponPluginApi.getCouponsAppliedByCouponCode(couponCode);

                if (null != couponsApplied) {
                    if (!couponsApplied.isEmpty()) {
                        List<JSONObject> jsonCouponsApplied = new ArrayList();
                        for (CouponsAppliedRecord couponApplied : couponsApplied) {
                            ...
                            jsonCouponsApplied.add(jsonCoupon);
                        }

                        JSONObject jsonResponse = JsonHelper.buildCouponsAppliedListJsonResponse(jsonCouponsApplied);
                        ...
                        errorMessage.put(Constants.ERROR, "There are no Coupons Applied with the specified Coupon code");
                    ...
                    logService.log(LogService.LOG_ERROR, "Error getting List of Coupons Applied from the DB");
                    JSONObject errorMessage = new JSONObject();
                    errorMessage.put(Constants.ERROR, "Can't get List of Coupons Applied from the DB. Response: null object");
                    ServletHelper.writeResponseToJson(response, errorMessage.toString());
                    buildResponse(response);
                }
            ...
            JSONObject errorMessage = new JSONObject();
            errorMessage.put(Constants.ERROR, "Coupon code is empty or not valid");
            ServletHelper.writeResponseToJson(response, errorMessage.toString());
            buildResponse(response);
        }
    }
