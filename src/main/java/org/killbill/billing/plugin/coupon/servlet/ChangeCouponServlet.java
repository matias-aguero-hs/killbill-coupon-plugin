...
public class ChangeCouponServlet extends PluginServlet {

    private final LogService logService;
    private final CouponPluginApi couponPluginApi;

    public ChangeCouponServlet(final LogService logService, final CouponPluginApi couponPluginApi)
    {
    ...
    /**
     * Method doPost will handle the changeCoupon operation
     * @param request
     * @param response
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (null == coupon || (null == coupon.getCouponCode() || coupon.getCouponCode().isEmpty())
                ...
                logService.log(LogService.LOG_INFO, "Calling CouponPluginAPI to get the stored information of the Coupon: " + coupon.getCouponCode());
                List<CouponsProductsRecord> oldCouponProducts = couponPluginApi.getProductsOfCoupon(coupon.getCouponCode());

                logService.log(LogService.LOG_INFO, "Calling CouponPluginAPI to Update a Coupon");
                couponPluginApi.updateCoupon(oldCouponProducts, coupon, context);

                logService.log(LogService.LOG_INFO, "Getting recently updated Coupon using couponCode: " + coupon.getCouponCode());
                CouponsRecord couponUpdated = couponPluginApi.getCouponByCode(coupon.getCouponCode());

                if (null != couponUpdated) {
                    ...
                    JSONObject jsonResponse = JsonHelper.buildCouponJsonResponse(couponUpdated);
